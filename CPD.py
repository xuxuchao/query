...
from api_key import get_token,timestamp_to_str,AESCipher
...
en_url = "https://10.1.19.222/opra-api/upl-file/test/decry?data={0}&type={1}"
de_url = "https://10.1.19.222/opra-api/upl-file/test/encry?data={0}&type={1}"

def headers_apikeys(url):
...
def get_re_en():
    ...
    while True:
        try:
            ...
            name_resp_en = requests.get(name_url_en,headers=headers_apikeys(name_url_en),verify=False)

            dicts['first_name'] = f'{first_name}'
            dicts_en['first_name'] = f'{name_resp_en.text}'
            # 名字解密
            name_url_re = de_url.format(f'{name_resp_en.text}','NAME')
            name_resp_re = requests.get(name_url_re,headers=headers_apikeys(name_url_re),verify=False)

            dicts_re['first_name'] = f'{name_resp_re.text}'
            # full名字加密
            full_name_url_en = en_url.format(f'{full_name}','NAME')
            full_name_resp_en = requests.get(full_name_url_en,headers=headers_apikeys(full_name_url_en),verify=False)
            ...
            full_name_resp_re = requests.get(full_name_url_re,headers=headers_apikeys(full_name_url_re),verify=False)
            dicts_re['full_name'] = f'{full_name_resp_re.text}'
            # 常旅客卡号加密
            frequent_flyer_no_url_en = en_url.format(f'{frequent_flyer_no_of_passenger}','FFP')
            frequent_flyer_no_resp_en = requests.get(frequent_flyer_no_url_en,headers=headers_apikeys(frequent_flyer_no_url_en),verify=False)
            ...
            frequent_flyer_no_resp_re = requests.get(frequent_flyer_no_url_re,headers=headers_apikeys(frequent_flyer_no_url_re),verify=False)
            ...
            credential_no_resp_en = requests.get(credential_no_url_en,headers=headers_apikeys(credential_no_url_en),verify=False)
            ...
            credential_no_resp_re = requests.get(credential_no_url_re,headers=headers_apikeys(credential_no_url_re),verify=False)
            dicts_re['credential_no'] = f'{credential_no_resp_re.text}'
            # 护照号加密
            passport_no_url_en = en_url.format(f'{passport_no}','IDENTITYCARD')
            passport_no_resp_en = requests.get(passport_no_url_en,headers=headers_apikeys(passport_no_url_en),verify=False)
            ...
            passport_no_resp_re = requests.get(passport_no_url_re,headers=headers_apikeys(passport_no_url_re),verify=False)
            ...
            for key, value in dicts.items():
                ...
                if dicts_en[key] == value and value !='' and key !='ticket_no' and key !='frequent_flyer_no_of_passenger':
...
if __name__ == '__main__':
    ...
    # file_name = 'MU_CPD_20230401.txt'
    file_name = "MU_CPD_20230402.txt"
