...
# --------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def reduction(self):
        ...
        if not carrier:
            ...
            update_sql = [
                f"UPDATE OPRAUPL.UPL_DCS_CPD SET TICKET_VARIANCE_IND='N',TICKET_MATCH_FLAG=null WHERE OPERATING_CARRIER='{carrier}' AND OPERATING_FLIGHT_NO='{flight_no}' AND  FLIGHT_FIRST_LEG_DEPARTURE_DATE=DATE '{flight_departure_date}' AND TICKET_VARIANCE_IND IN ('Y','N') {end_sql}",
